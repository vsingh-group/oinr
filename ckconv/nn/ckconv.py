...
class CKConvBase(torch.nn.Module):
	def __init__(
		...
		**kwargs,
		...
		self.downsample_size = kwargs.pop("downsample_size", 1) # scales both kernel size and stride for PSF
...
class CKConv(CKConvBase):
	def __init__(
		...
		**kwargs,
		...
		print("Initialize new 2D continous convolution kernel")

	def forward(self, x):
		# 1. Construct kernel
		conv_kernel = self.construct_kernel(x)
		# 4. Compute convolution & return result
		return self.conv(x, conv_kernel, self.bias, separable=False, causal=self.causal, downsample_size=self.downsample_size)